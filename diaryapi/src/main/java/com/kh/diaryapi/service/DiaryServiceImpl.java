...
public class DiaryServiceImpl implements DiaryService {
    ...
    public Long register(DiaryDTO diaryDTO) {
        ...
        return savedDiary.getTno();
    }

    @Override
    public DiaryDTO get(Long tno) {
        java.util.Optional<Diary> result = diaryRepository.findById(tno);
    ...
    public void remove(Long tno) {
        diaryRepository.deleteById(tno);
    }
    ...
    public PageResponseDTO<DiaryDTO> list(PageRequestDTO pageRequestDTO) {
        ...
        List<DiaryDTO> dtoList = result.getContent().stream().map(todo -> modelMapper.map(todo, DiaryDTO.class))
